...
equation
   ...
   // Spec. says it is not allowed to vectorize these (all ops except + and  - actually, it's not clear on the logical ones (and, or, not)).
   // OMC expands them only as a last choice (no matching array op overlad func found) 
   // but with a warning!! (CAN be prevented any time if neccessary)
   c1 = c1 / c2;  // WARNING!!
   b = c1 == c2;  // WARNING!!
   c1 = c1 ^ c2;  // WARNING!!
   ...
   // This is tricky!
   c1 = c1 * {1,2,3}; // This should call Complex.'*'.scalarProduct after const. an array (We have a match for an array op -> priority) 
   ...
   b = c1 and c2;  // This should be vectorized (no match for an array op. )  (!!WITH a WARNING!!)
...
//   c2[2] = Complex.'^'(c1[2],c1[2]);
//   c2[3] = Complex.'^'(c1[3],c1[3]);
//   c2 = Complex.'*'.scalarProduct({c1[1],c1[2],c1[3]},{c1[1],c1[2],c1[3]});
//   c2[1] = Complex.'+'(c1[1],c1[1]);
//   c2[2] = Complex.'+'(c1[2],c1[2]);
//   c2[3] = Complex.'+'(c1[3],c1[3]);
...
//   b[1] = Complex.'=='(c1[1],c2[1]);
//   b[2] = Complex.'=='(c1[2],c2[2]);
//   b[3] = Complex.'=='(c1[3],c2[3]);
//   c1[1] = Complex.'^'(c1[1],c2[1]);
//   c1[2] = Complex.'^'(c1[2],c2[2]);
//   c1[3] = Complex.'^'(c1[3],c2[3]);
//   c1 = Complex.'*'.scalarProduct({c1[1],c1[2],c1[3]},{Complex.'constructor'.fromReal(1.0,0.0),Complex.'constructor'.fromReal(2.0,0.0),Complex.'constructor'.fromReal(3.0,0.0)});
